...
With no cheap prospect performing to an extremely high level, we can settle for a player like Khris Middleton or Jayson Tatum, or we can go for the stars and use our salary cap on the likes of Kevin Durant or Paul George. 
...
```{r kemba-walker, echo = FALSE, warning = FALSE}
selected_player_pg_2 <- pg %>%
  filter(player_name == "Kemba Walker") # Filter for the chosen PG

datatable(selected_player_pg_2, rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T))
```
...
```{r donovan-mitchell, echo = FALSE, warning = FALSE}
selected_player_sg <- sg %>%
  filter(player_name == "Donovan Mitchell") # Filter for the chosen SG

datatable(selected_player_sg, rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T))
```
...
```{r kevin-durant, echo = FALSE, warning = FALSE}
selected_player_sf <- sf %>%
  filter(player_name == "Kevin Durant") # Filter for the chosen SF

datatable(selected_player_sf, rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T))
```
...
Do not forget... More Points = More Wins.
This man does this and more. 
...
```{r julius-randle, echo = FALSE, warning = FALSE}
selected_player_pf_2 <- pf %>%
  filter(player_name == "Julius Randle") # Filter for the chosen PF

datatable(selected_player_pf_2, rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T))
```
...
```{r karl-anthony-towns, echo = FALSE, warning = FALSE}
selected_player_c <- centres %>%
  filter(player_name == "Karl-Anthony Towns") # Filter for the chosen C

datatable(selected_player_c, rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T))
```
